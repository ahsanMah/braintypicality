...
INLIER_COHORTS = ["ABCD", "LR-Typical"]
OOD_COHORTS = [c for c in COHORTS if c not in INLIER_COHORTS]
...
def get_region_scores():
    region_scores = pd.read_csv(
        f"{HEATMAP_DIR}/roi/AAL+CSF_roi_scores.csv", index_col="ID"
    )
    cohort_renamer = {
...
def load_global_score_data() -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    # abcd_data = np.concatenate([score_file[f] for f in ['inlier_score_norms']], axis=0)

    score_file = np.load(
        f"{SCORE_DIR}/abcd-val_abcd-test_lesion_load_20_results.npz", allow_pickle=True
    )
    abcd_data = np.concatenate(
        [score_file["eval_score_norms"], score_file["inlier_score_norms"]], axis=0
    )
...
def plot_demographics(index):
    ...
    return selected.hvplot(kind="bar").opts(
        height=ROI_PLOT_HEIGHT - HEATMAP_HEIGHT - 50, title="Cohort Distribution"
    )
...
def plot_roi_scores(index, quantile_threshold=60, show_bars_max=80):
    ...
    roi_data["Color"] = roi_data["ROI"].apply(
        lambda r: "pink" if roi_median[r] > quantile_threshold else "lightgrey"
    )
    ...
    boxplot = boxplot.relabel("ROI Scores").opts(
        opts.Overlay(
            min_width=ROI_PLOT_WIDTH,
            min_height=ROI_PLOT_HEIGHT,
            show_legend=False,
            xlim=(0, 100),
        )
        # height=ROI_PLOT_HEIGHT
...
cbcl_cols = list(
    filter(
        lambda c: re.match(".*_T_score", c),
        ibis_metadata.columns,
    )
)
vineland_cols = list(
    filter(lambda c: re.match(".*Vine.*STD_SCORE", c), ibis_metadata.columns)
)
...
########## SOM ############
data, score_target, sample_ids = (
    load_global_score_data() if LOAD_GLOBAL_SCORES else load_lobe_score_data()
)
inlier_labels_idx = [cohort_to_idx[c] for c in INLIER_COHORTS]
inlier_mask = np.isin(score_target, inlier_labels_idx)
inlier_data = data[inlier_mask]

som, distance_map = build_simpsom_map(
    inlier_data, net_height=GRID_ROWS, net_width=GRID_COLS
)
...
for idx in range(0, data.shape[0]):
    ...
    if COHORTS[label_idx] not in INLIER_COHORTS:
...
for pos, cell in cell_groups:
    ...
    y0 = ys + y
    y1 = ys + y
    for i, c in enumerate(OOD_COHORTS):
        if c in counts:
...
for j in range(GRID_COLS):
    for i in range(GRID_ROWS):
        ...
        if (j, i) in cell_groups.groups:
            counts.update(
                cell_groups.get_group((j, i))["Cohort"].value_counts().to_dict()
            )
        # print(j,i, counts)
...
bplots_config = {
    ...
    "ADOS": [
        ados_cols,
        np.arange(
            METADATA_DF[ados_cols].values.min(), METADATA_DF[ados_cols].values.max(), 2
        ),
    ],
}
